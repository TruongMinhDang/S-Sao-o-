...
// ================== PAGE COMPONENT (ĐÃ CẬP NHẬT) =====================
export default function MyClassPage() {
  // --- STATE MANAGEMENT --- 
  const { user: currentUser, isAdmin, isHomeroomTeacher } = useAuth(); // Lấy thêm role
  ...
  useEffect(() => {
    ...
    const fetchInitialData = async () => {
      try {
        ...
        // --- LOGIC MỚI: TỰ ĐỘNG CHỌN LỚP CHỦ NHIỆM ---
        // Chỉ tự động chọn nếu là GVCN
        ...
        // Admin và các role khác sẽ bắt đầu với việc chọn thủ công
    ...
    fetchInitialData();
  }, [currentUser, isHomeroomTeacher]); // Cập nhật dependency

useEffect(() => {
  if (selectedClassId === 'all') {
      setViolations([]);
      setLoading(false); // Đảm bảo loading kết thúc nếu không có lớp nào được chọn
      return;
  }
...
const availableGrades = useMemo(() =>
 Array.from(new Set(allClasses.map(c => c.gradeId).filter(Boolean))).sort(),
 [allClasses]
);

// CẬP NHẬT: Admin thấy hết lớp, người khác lọc theo khối
const availableClasses = useMemo(() => {
  if (isAdmin || selectedGrade === 'all') return allClasses;
...
const chartData = useMemo(() => {
  ...
  const data = allTermWeeks.map(weekNumber => {
      const reportForWeek = weeklyReports.find(r => r.id === `week_${weekNumber}`);
      const score = reportForWeek?.scores?.[selectedClass.name];
      return { week: weekNumber, points: typeof score === 'number' ? score : null };
  });
  let lastKnownScore: number | null = 1000; // Khởi tạo điểm ban đầu là 1000
  return data.map(d => {
      if (d.points !== null) { lastKnownScore = d.points; }
      return { ...d, points: d.points === null ? lastKnownScore : d.points };
  });
      ...
      <CardHeader>
        <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            {/* CẬP NHẬT: Chỉ hiển thị bộ lọc khối cho non-admin */}
            {!isAdmin && (
              <Select onValueChange={setSelectedGrade} value={selectedGrade}>
                ...
                <SelectContent className="bg-popover">
                  ...
                  const gradeLabel = String(g).split('_')[1] || g;
